...
public class AntecedentesMedicosService {
    ...
    /**
     * Métodos nuevos que crean entidades y devuelven los IDs generados
     */
    public List<Integer> createAlergiasWithIds(List<Alergia> alergias) throws SQLException {
        List<Integer> ids = new ArrayList<>();
        if (alergias != null && !alergias.isEmpty()) {
            for (Alergia alergia : alergias) {
                int id = alergiaService.create(alergia);
                ids.add(id);
            }
        }
        return ids;
    ...
    public List<Integer> createMedicamentosCronicosWithIds(List<MedicamentoCronico> medicamentos) throws SQLException {
        List<Integer> ids = new ArrayList<>();
        if (medicamentos != null && !medicamentos.isEmpty()) {
            for (MedicamentoCronico medicamento : medicamentos) {
                int id = medicamentoCronicoService.create(medicamento);
                ids.add(id);
            }
        }
        return ids;
    ...
    public List<Integer> createCirugiasWithIds(List<RegistroCirugia> cirugias) throws SQLException {
        List<Integer> ids = new ArrayList<>();
        if (cirugias != null && !cirugias.isEmpty()) {
            for (RegistroCirugia cirugia : cirugias) {
                int id = registroCirugiaService.create(cirugia);
                ids.add(id);
            }
        }
        return ids;
    ...
    public List<Integer> createEnfermedadesCronicasWithIds(List<RegistroEnfermedadCronica> enfermedades) throws SQLException {
        List<Integer> ids = new ArrayList<>();
        if (enfermedades != null && !enfermedades.isEmpty()) {
            for (RegistroEnfermedadCronica enfermedad : enfermedades) {
                int id = registroEnfermedadCronicaService.create(enfermedad);
                ids.add(id);
            }
        }
        return ids;
